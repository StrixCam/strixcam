...
HttpDownloadServer::HttpDownloadServer(std::string bind_address, std::uint16_t port,
    ...
    server_->Get("/recordings/(.*)", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        res.set_content_provider(
            size, "video/mp4",
            [file](std::size_t offset, std::size_t length,  // NOLINT(bugprone-easily-swappable-parameters) // floor-ok: (offset,length) order fixed by httplib ContentProvider callback contract
