...
struct CameraConfig {
    ...
    // the 1080p60 binned mode (mode 1) — measured on metal 2026-07-10 (~2x the
    // real detail at equal JPEG size). The CPU-side pipeline still only ever sees
    // the delivered 1080p; only the ISP + VIC (hardware) run at 4K.
    ...
    // pipeline — the sensor 4K frame is VIC-downscaled to this before it costs any
    // CPU-side work.
    ...
    // delivered width/height below: the capture pipeline VIC-downscales sensor ->
